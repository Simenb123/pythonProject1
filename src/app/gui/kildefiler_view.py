...
try:
    ...
    from app.services.clients import load_meta, save_meta, list_years, open_or_create_year, set_default_year
...
class KildefilerView(ttk.Frame):
    ...
    def __init__(
        ...
        holder.year_var = self.year_var  # for context (not used by VersionsPanel directly)
    ...
    def _ensure_mapping_now(self) -> None:
        ...
        if not (self.root_dir and resolve_active_raw_file and read_raw and ensure_mapping_interactive):
            messagebox.showwarning("Ikke tilgjengelig", "Mapping-funksjonen er ikke tilgjengelig i denne konteksten.")
            return
        src = self.source_var.get()
        year = self.year_var.get()
        # Attempt to find active version file
        p = resolve_active_raw_file(self.root_dir, self.client_name, year, src, "interim", self.meta) or \
            resolve_active_raw_file(self.root_dir, self.client_name, year, src, "ao", self.meta)
        if not p:
    ...
    def _start_bilag(self, modus: str) -> None:
        ...
        if not self.root_dir:
            messagebox.showwarning("Ikke tilgjengelig", "Bilagsfunksjonen er ikke tilgjengelig i denne konteksten.")
            return
        src = self.source_var.get()
        year = self.year_var.get()
        # Build command; always pass type=interim for now
        args = [sys.executable, "-m", "app.gui.bilag_gui_tk",
                f"--client={self.client_name}", f"--year={year}", f"--source={src}", "--type=interim", f"--modus={modus}"]
        try:
    ...
    def _open_orgchart(self) -> None:
        ...
        try:
            ...
            messagebox.showinfo("Eierskap", "Orgkart-funksjonen er ikke tilgjengelig i denne konteksten.")
            return
