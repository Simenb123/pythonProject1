...
- Finner direkte og indirekte relasjoner via registry_db
- Fuzzy navn med rapidfuzz hvis tilgjengelig, ellers difflib
"""
...
from .registry_db import (
    open_db, get_owners, companies_owned_by, get_company_name, search_name_candidates, normalize_orgnr, normalize_name
)
...
def fuzzy_match_name_against_registry(conn, name: str, min_score:int=90, limit:int=20) -> List[Tuple[str,str,int]]:
    """
    Returner [(reg_navn, reg_orgnr, score)] for topp kandidater i registeret som matcher 'name'.
    Først henter vi kandidater fra DB (FTS/LIKE), deretter rangerer vi med _score.
    """
    name_n = normalize_name(name)
    if not name_n:
        return []
    cands = search_name_candidates(conn, name_n, limit=limit*5)  # få bredere utvalg før ranking
    ranked = [(n, o, _score(name_n, n)) for (n,o) in cands]
...
def relations_for_client(conn, client_orgnr: str, client_name: str, client_orgs_set: set[str],
    ...
    """
    Finn alle relevante relasjoner for én klient:
    - Eiere av klienten
    - Selskaper klienten eier
    - Kryss: eier/selskap finnes i klientlista
    - Fuzzy navnesøk dersom klientnavn mangler orgnr i registeret
    """
    ...
    for r in owners:
        rel = {
            ...
            "direction": "owned_by",  # klienten eies av ...
    ...
    # 3) Fuzzy navn (hvis klientnavn kan ha treff i registeret)
